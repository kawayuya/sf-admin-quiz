...
import { InsertUser, users, InsertQuizResult, quizResults, passwordResetTokens } from "../drizzle/schema-pg";
...
export async function createQuizResult(data: InsertQuizResult) {
  ...
  try {
    const result = await db.insert(quizResults).values(data).returning({ id: quizResults.id });
...
export async function deleteExpiredPasswordResetTokens(): Promise<void> {
  ...
  try {
    await db.delete(passwordResetTokens).where(ne(passwordResetTokens.expiresAt, new Date()));
  } catch (error) {
