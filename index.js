...
var users = mysqlTable("users", {
  ...
  /** Hashed password for email/password authentication (null if using OAuth only) */
...
var quizResults = mysqlTable("quizResults", {
  id: int("id").autoincrement().primaryKey(),
...
var ENV = {
  ...
  cookieSecret: process.env.JWT_SECRET ?? "sf-admin-quiz-default-secret-key-for-development",
...
async function createQuizResult(data) {
  ...
  try {
    ...
    const result = await db.select().from(quizResults).orderBy(desc(quizResults.createdAt)).limit(1);
...
async function getQuizResultById(id, userId) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    ...
    if (result[0].userId !== userId) return void 0;
    return result[0];
  } catch (error) {
...
async function getWrongQuestionIds(userId) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    const results = await db.select({ wrongQuestionIds: quizResults.wrongQuestionIds }).from(quizResults).where(eq(quizResults.userId, userId));
    const allWrongIds = /* @__PURE__ */ new Set();
    results.forEach((r) => {
      if (r.wrongQuestionIds) {
        try {
          const ids = JSON.parse(r.wrongQuestionIds);
...
var appRouter = router({
  ...
  quiz: router({
    saveResult: protectedProcedure.input(
      z2.object({
        mode: z2.enum(["normal", "weakPoints"]),
        selectedCategories: z2.array(z2.string()).optional(),
        totalQuestions: z2.number(),
        correctAnswers: z2.number(),
        score: z2.number(),
        categoryResults: z2.record(z2.string(), z2.object({ correct: z2.number(), total: z2.number() })),
        wrongQuestionIds: z2.array(z2.number()),
    ...
    getResult: protectedProcedure.input(z2.object({ id: z2.number() })).query(({ ctx, input }) => {
      return getQuizResultById(input.id, ctx.user.id);
    }),
    getWrongQuestionIds: protectedProcedure.query(({ ctx }) => {
      return getWrongQuestionIds(ctx.user.id);
    })
  })
});
